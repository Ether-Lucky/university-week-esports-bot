...
| Only the owner sees the **staff** commands (`/setup`, `/event`, `/application`, `/mechanics`, `/dashboard`, `/player`, `/announce`, …) | Those are gated to Discord's **Manage Server** permission for visibility. Give the **E-Sports Head** role that permission: Server Settings → Roles → **E-Sports Head** → enable **Manage Server**. Now Head-role holders see and use them (the bot still restricts each command to Head/staff at runtime). To reveal them to Committee/OIC/FIC too, either grant those roles Manage Server, or map roles per-command in Server Settings → Integrations → the bot. |
| Members/applicants can't see the **member** commands (`/team`, `/findteam`, `/tryout`, `/switch-game`, `/recruit`) | Those aren't gated. Make sure `@everyone` (and Applicant/Audience) have **Use Application Commands** enabled (Server Settings → Roles, and not denied on the channel), and that the bot has been restarted and refreshed (Ctrl+R). |
